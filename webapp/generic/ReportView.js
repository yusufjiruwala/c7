    ...
    function (QueryView) {
        ...
        ReportView.prototype.parseRep = function (json) {
            ...
            for (var r = 0; r < rps.length; r++) {
                var rep = {};
                rep.name = rps[r].name;
                rep.descr = Util.nvl(rps[r].descr, rps[r].name);
                rep.code = rps[r].code;
                rep.rptNo = r;
                ...
                for (var i in qrys) {
                    ...
                    if (typeof qr.dispRecords == "object") {
                        var newr = "L";
                        ...
                        if (sap.ui.Device.resize.height > 1080)
                            newr = "XXL";
                        console.log("DEVICE " + newr + " -height=" + sap.ui.Device.resize.height + " records=" + qr.dispRecords[newr]);

                        qr.dispRecords = qr.dispRecords[newr];
                    }
                    qr.fields = {};
        ...
        ReportView.prototype.helperFunctions = {
            ...
            mainPop: {
                ...
                createView: function (pg, pRptNo) {
                    ...
                    this.lstRepTbl.getControl().addStyleClass("sapUiSizeCondensed reportTable");
                    this.lstRepTbl.getControl().setSelectionBehavior(sap.ui.table.SelectionBehavior.Row);
                    ...
                    for (var i = 0; i < rps.length; i++) {
                        ...
                        var dt = {
                            "CODE": rps[i].name,
                            "TITLE": Util.nvl(rps[i].descr, rps[i].name),
                    ...
                    this.lstRepTbl.mLctb.cols[
                        ...
                        .mUIHelper.display_width = "225";
                    this.lstRepTbl.mLctb.cols[
                        this.lstRepTbl.mLctb.getColPos("TITLE")]
                        .mUIHelper.display_width = "500";
                    ...
                    this.pg.addContent(new sap.m.Title({ text: "Select Sub Report of " + thatRV.title }).addStyleClass("sapUiMediumMarginTop sapUiLargeMarginBegin"));
                ...
                showDispInMaster: function (pRptNo) {
                    ...
                    for (var f in flds)
                        if (Util.nvl(flds[f].canvas, "") == "dispInPara") {
                            ...
                            flds[f].obj.mainObj.parentObj = flds[f].obj;
                    ...
                    this.cmdExe = new sap.m.Button({
                        ...
                        // enabled: false,
                        press: function () {
                            if (thatForm.status_of_rep == undefined) return;
                            if (Util.nvl(thatForm.status_of_rep, "") != "END" && Util.nvl(thatForm.status_of_rep, "") != "START") {
                                for (var fld in paras) {
                                    var vl = UtilGen.getControlValue(paras[fld]);
                                    UtilGen.setControlValue(paras[fld].paraObj, vl, vl, true);
                                }
            ...
            view:
            {
                ...
                showRepHeaderHTML: function () {
                    ...
                    var pvl = "";
                    for (var f in flds) {
                        pvl = flds[f].obj.getValue();
                        if (flds[f].data_type == FormView.DataType.Date)
                            pvl = sf.format(UtilGen.getControlValue(flds[f].obj));
                        if (UtilGen.nvl(pvl, "") != "")
                            h2 += "<div class='paras'><td class='paraLabel'>" + ReportView.getTitleFromField(flds[f]) + "</td> : <td class='paraText'>" + pvl + "</td></div>";
                    }
                    ht += h2;
                    setTimeout(function () {
                        thatRV.tbMain.$().css("background-color", "lightblue");
                ...
                createReport: function (rep) {
                    ...
                    for (var q in rep.db) {
                        ...
                        if (rep.db[q].showType == FormView.QueryShowType.FORM) {
                            ...
                            if (Object.keys(qr.fields).length > 0 && qr.fields[Object.keys(qr.fields)[0]].grouped) {
                                ...
                                thatRV.lstRepGroups = UtilGen.createControl(sap.m.ComboBox, thatRV.view, "lstRepGroup" + thatRV.timeInLong, {
                                    customData: [{ key: "" }],
                                    width: "40%",
                                    items: {
                                        path: "/",
                                    ...
                                    selectionChange: function (event) {
                                        thatRV.helperFunctions.misc.doQuickFilterData(qr);
                                    }
                                }, "string", undefined, undefined, "@");
                                thatRV.lstRepGroups.addStyleClass("reportTable");
                                // Util.fillCombo(thatRV.lstRepGroups, cdt);
                                // this.scrollObjs.push(thatRV.lstRepGroups);
                                rep.dispCanvases[qr.canvas].push(thatRV.lstRepGroups);
                            }
                    ...
                    for (var ci in rep.dispCanvases) {
                        ...
                        if (thatRV.objs[ci].canvasType == ReportView.CanvasType.FORMCREATE2) {
                            thatRV.objs[ci].obj = UtilGen.formCreate2("", true, rep.dispCanvases[ci], {}, sap.m.ScrollContainer, {
                                width: {
                                    "S": 400,
                                    "M": 600,
                                    "L": 800
            ...
            load: {
                ...
                fetchQuery: function (qryName, execBeforeSql) {
                    ...
                    } else if (qryObj.hasOwnProperty("bat7CustomAddQry") && qryObj.bat7CustomAddQry != undefined)
